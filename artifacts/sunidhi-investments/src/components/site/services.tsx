        ...
        <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:mt-12 lg:grid-cols-4">
          {siteConfig.services.map(({ slug, title, description, icon: Icon, ctaLabel }) => (
            <article key={slug} className="service-card interactive-lift touch-press group rounded-lg border border-border/80 bg-card p-4 transition-all duration-300 sm:p-5 lg:p-6">
              ...
              <a href="#contact" className="touch-target mt-3 flex items-center gap-2 rounded-md text-xs font-bold text-primary transition-colors hover:text-accent sm:mt-4" data-testid={`link-service-${slug}`}>{ctaLabel} <ArrowRight size={14} /></a>
            </article>
          ))}
        </div>
      </Container>
